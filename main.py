...
class MyServer(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if(len(actions)!=3):
            self.wfile.write(bytes("wrong paramethers number <br>", "utf-8"))
            ...
            githandler = gitRepositoryHandler.gitRepo(actions[1])
            repoList = githandler.listRepos()
            for row in repoList:
            ...
            githandler = gitRepositoryHandler.gitRepo(actions[1])
            totalStars = githandler.getStars()
